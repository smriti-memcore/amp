...
| **`amp.batch_encode`** | v1.2-draft, **landed (reference impl)** | Multi-row encode in a single round-trip under one shared scope. Schema + spec §3.2.5 + reference-server implementation + 13 compliance tests. Per-row partial-failure semantics; ordering preserved; backend cap of 1000 entries; oversize requests rejected with `invalid_request`. |
| **Metadata filters in `RecallFilters`** | v1.2-draft, **landed (reference impl)** | `MetadataFilter` predicate (`eq` / `ne` / `gt` / `gte` / `lt` / `lte` / `in` / `contains`) over reserved-vocabulary and user-defined keys; strict-AND composition across multiple entries; missing keys and type mismatches are silent misses (never raise). Schema + spec §3.2.2.1 + reference-server implementation + 16 compliance tests. |
