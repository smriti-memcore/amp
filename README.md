<p align="center">
  <a href="https://github.com/smriti-memcore/amp">
    <img src="AMPLogo.png" width="400px" alt="AMP - Agent Memory Protocol">
  </a>
</p>

<p align="center" style="display: flex; justify-content: center; gap: 10px; align-items: center; flex-wrap: wrap;">
  <a href="https://github.com/smriti-memcore/amp/blob/main/LICENSE">
    <img src="https://img.shields.io/badge/license-MIT-green.svg?style=flat-square" alt="License">
  </a>
  <a href="https://github.com/smriti-memcore/amp/actions">
    <img src="https://img.shields.io/github/actions/workflow/status/smriti-memcore/amp/spec-validation.yml?branch=main&style=flat-square&label=compliance%20tests" alt="Spec Validation Status">
  </a>
  <a href="https://pypi.org/project/amp-server/">
    <img src="https://img.shields.io/pypi/v/amp-server?color=%2334D058&label=pypi%20package&style=flat-square" alt="amp-server PyPI version">
  </a>
</p>

<p align="center">
  <a href="spec/amp-v1.1.md"><strong>📄 Read the Specification (v1.1) →</strong></a>
  ·
  <a href="docs/MIGRATING-v1.0-to-v1.1.md"><strong>Migration Guide (v1.0 → v1.1) →</strong></a>
</p>
...
**AMP v1.1** is a dual-delivery service protocol: it provides a standalone REST API (with an OpenAPI 3.0 contract) and a parallel **Model Context Protocol (MCP)** tool adapter, sharing a single set of JSON-Schema definitions. 

Application harnesses manage context (prompt injections, background compaction, multi-tenant partitioning) programmatically over the REST channel, while the same backend exposes clean cognitive tools directly to the LLM agent over the MCP channel.

---

## ✨ Key Features & Protocol Strengths
...
* **🛡️ Multi-dimensional scoping.** Memories partition across `org_id`, `app_id`, `user_id`, `session_id`, `agent_id`, `group_id`, and `workspace_id`. At least one *isolating* key (`agent_id` / `group_id` / `workspace_id` / `user_id`) is required. Hierarchical queries work while strict-AND semantics prevent silent scope-elevation across partitions.
...
* **📦 MXF as a callable contract.** `amp.export` / `amp.import` are first-class protocol verbs. You can round-trip a Memory Exchange Format (MXF) document between any two Full-conformant backends with no vendor adapter. Conflict policies (`skip` / `overwrite` / `fail_atomic` / `fail_fast`) and scope-remap policies (`strict` / `inherit`) are explicit.
* **🏷️ Reserved metadata vocabulary.** `amp.ttl`, `amp.confidence`, `amp.entities`, `amp.relations`, `amp.categories`, `amp.summary` — namespaced reserved keys so backends interoperate on common fields without colliding with proprietary metadata.
* **🕰️ v1.0 backward compatibility.** Flat `agent_id` + `private` + `visibility` continue to work for one minor revision. The two conformant migration mappings (Strategy A — Agent-private; Strategy B — User-private) are defined in [spec §5](spec/amp-v1.1.md#5-backward-compatibility--deprecations); a walkthrough lives in [docs/MIGRATING-v1.0-to-v1.1.md](docs/MIGRATING-v1.0-to-v1.1.md). Removal targeted at v1.2.
...
### ⚙️ Harness-facing system verbs (should NOT be projected to the LLM)
...
### 1. Run the reference server (Full-conformant, MCP stdio)
...
### 2. Run the Mem0 wrapper (Core-conformant, MCP stdio)
...
### 3. Run the SuperMemory wrapper (Core-conformant, MCP stdio)

If you use SuperMemory as your memory backend, you can run the AMP wrapper:

```bash
# One-shot run with uvx:
export SUPERMEMORY_API_KEY="your-supermemory-api-key"
uvx --from "git+https://github.com/smriti-memcore/amp.git#subdirectory=python/amp-supermemory-wrapper" amp-supermemory-wrapper
...
```
See [python/amp-supermemory-wrapper/README.md](python/amp-supermemory-wrapper/README.md) for Claude Desktop wiring.

### 4. Run the compliance suite against your backend
...
├── python/amp-supermemory-wrapper/          # Core-conformant wrapper for SuperMemory
...
**v1.2-draft — In progress.** Spec extensions tracked in [spec/amp-v1.1.md](spec/amp-v1.1.md). 

**Landed so far:** 
* Appendix C (REST routing + gRPC `MemoryService` Protobuf contract)
* `amp.provenance.*` / `amp.lineage.*` reserved metadata keys
* `amp.update` (RFC 7396 merge-patch semantics)
* `amp.batch_encode` (multi-row ingest under one shared scope, per-row partial-failure semantics)
* Structured `metadata_filters` in `RecallFilters` (`eq` / `ne` / `gt` / `gte` / `lt` / `lte` / `in` / `contains` over reserved-vocabulary and user-defined keys, strict-AND composition). 

Compliance suite now at 154 tests, all passing. v1.1-conformant backends remain conformant — v1.2-draft adds capability without breaking the v1.1 surface.
