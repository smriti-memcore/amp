...
**154 tests; all pass on the current implementation.** Coverage includes Core verbs, scope validation, error mapping (§3.5), MCP tool annotations (§3.4), MXF export round-trips, cursor resumability, all four `on_conflict` policies (including the `not_supported` return for `fail_atomic`), both `scope_remap` modes, the v1.2-draft `amp.update` verb (content / metadata merge / null-delete / replace mode / no-op / scope isolation / idempotency / RFC 7396 nested null-delete against absent and non-object targets / transactional rollback on save failure / 64 KiB metadata-bag cap), the v1.2-draft `amp.batch_encode` verb (all-succeed / mixed partial failure / ordering / summary counts / scope sharing / oversize rejection / per-row metadata / row-level strictness on `force` / `source` / forbidden keys / unknown keys / durable-save honesty), the v1.2-draft `metadata_filters` predicate on `amp.recall` (all 8 operators, strict-AND composition, missing-key & type-mismatch as silent miss including `ne` against type-mismatched values, request-level validation surface incl. null/non-scalar rejection and `maxItems=32` cap), the `top_k`-under-post-retrieval-filtering oversampling rule, and `timestamp_after` / `timestamp_before` recall filters.
